...
export type PhoenixdNode = {
  url: string;
  password: string;
};

/**
 * Generated factory for {@link PhoenixdNode} record objects.
 */
export const PhoenixdNode = (() => {
  const defaults = () => ({});
  const create = (() => {
    return uniffiCreateRecord<PhoenixdNode, ReturnType<typeof defaults>>(
      defaults
    );
  })();
  return Object.freeze({
    /**
     * Create a frozen instance of {@link PhoenixdNode}, with defaults specified
     * in Rust, in the {@link lni} crate.
     */
    create,
     ...
     * Defaults specified in the {@link lni} crate.
     */
    defaults: () => Object.freeze(defaults()) as Partial<PhoenixdNode>,
  });
})();

const FfiConverterTypePhoenixdNode = (() => {
  type TypeName = PhoenixdNode;
  class FFIConverter extends AbstractFfiConverterArrayBuffer<TypeName> {
    read(from: RustBuffer): TypeName {
      return {
        url: FfiConverterString.read(from),
...
function uniffiEnsureInitialized() {
  ...
  const scaffoldingContractVersion =
    nativeModule().ffi_lni_uniffi_contract_version();
