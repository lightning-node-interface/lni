use lni::{
  speed::lib::SpeedConfig, CreateInvoiceParams, LookupInvoiceParams, PayInvoiceParams,
};
...
impl SpeedNode {
  ...
  pub fn lookup_invoice(&self, params: LookupInvoiceParams) -> napi::Result<lni::Transaction> {
    let txn = lni::speed::api::lookup_invoice(
      &self.inner,
      params.payment_hash,
      None,
      None,
  ...
  pub fn list_transactions(
    ...
    let txns = lni::speed::api::list_transactions(&self.inner, params.from, params.limit, params.search)
      .map_err(|e| napi::Error::from_reason(e.to_string()))?;
    Ok(txns)
  }
  ...
  pub fn on_invoice_events<T: Fn(String, Option<lni::Transaction>) -> Result<()>>(
    ...
    lni::speed::api::poll_invoice_events(&self.inner, params, move |status, tx| {
